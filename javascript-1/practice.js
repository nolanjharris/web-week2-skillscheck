...
/* Create an object called 'me' that has these keys with the correct values: firstName, state, age, greeter. firstName will be your name as a string. state will be your current state or providence as a string. age will be your age as a number. greeter will be a 
method that returns the string 'Hello! My name is NAMEVALUE and I live in STATEVALUE' with the correct values */
...
const me = {
	...
	age: 29,
	greeter: function () {
		return `Hello! My name is ${me.firstName} and I live in ${state}`;
	}
}
...
If the number is GREATER than 100, push 'big' as a string to the answers array. If the number is LESS than or EQUAL 
to 100, push 'small' as a string to the answers array. Return the answers array inside of the function */

// Code here
function bigOrSmall(arr) {
	let answers = [];
	arr.map(a => a > 100 ? answers.push('big') : answers.push('small'));
	return answers;
}
...
// Code Here
let doubled = myNumbers.map(a => a * 2);
...
// Code Here
let filtered = myNumbers.filter(a => a > 100);
...
// Code Here
let total = myNumbers.reduce((a, v) => a + v);

//////////////////PROBLEM 17////////////////////

/* Last, lets use .forEach to find the index of each item in the array. First, create an empty 
array called myNumbersIndex. Next, using forEach, push each items index from the myNumbers array to the myNumbersIndex array */

// Code Here
let myNumbersIndex = [];
myNumbers.forEach((a, i) => myNumbersIndex.push(i));
...
// Code Here
const forTheLoveOfGeorge = notGeorge.map(a => a = "George");
...
// Code Here
const enemies = people.filter(a => a.friend === false);
...
// Code Here
const totallyAwesome = people.reduce((a, v) => {
	return a + v.awesomeLevel
}, 0);